...
(() => {
  ...
  var PanZoomConfiguration = class {
    static configurePanContext(canvas) {
      this._panContext = (0, import_panzoom.default)(document.querySelector("#viewport"), {
  ...
  var NodeConfiguration = class _NodeConfiguration extends EventTarget {
    ...
    constructor(recipe, machine) {
      ...
      _NodeConfiguration._modalContainer.querySelector(".modal-window").addEventListener("click", (event) => {
        event.stopPropagation();
      });
      _NodeConfiguration._modalContainer.addEventListener("click", (event) => {
        event.stopPropagation();
        this.closeConfigurationWindow();
      });
    }
  ...
  var HelpModal = class _HelpModal {
    constructor() {
      ...
      _HelpModal._modalContainer.querySelector(".modal-window").addEventListener("click", (event) => {
        event.stopPropagation();
      });
      _HelpModal._modalContainer.addEventListener("click", (event) => {
        event.stopPropagation();
        this.closeModal();
      });
    }
  ...
  // src/RecipeSelectionModal.ts
  var RecipeSelectionModal = class _RecipeSelectionModal extends EventTarget {
    static recipeConfirmedEvent = "recipe-confirmed";
    static recipeSelectedEvent = "recipe-selected";
    static recipesTabSwitchedEvent = "recipes-tab-switched";
    constructor() {
      super();
      this._gameVersionText.innerText = `game version: ${Satisfactory_default.gameVersion}`;
      ...
      this.addEventListener(_RecipeSelectionModal.recipeSelectedEvent, () => {
      ...
      window.addEventListener("keydown", (event) => {
        ...
        if (event.key === "Enter" && this._isOpened && this.selectedRecipe != void 0) {
          event.preventDefault();
          event.stopPropagation();
          this.confirmRecipe();
        }
    ...
    setupTabs() {
      for (const machine of Satisfactory_default.machines) {
        let tabSelector = this.createHtmlElement("div", "tab-selector");
        tabSelector.title = machine.displayName;
        let machineIcon = this.createHtmlElement("img", "machine-icon");
        machineIcon.src = satisfactoryIconPath(machine.iconPath);
        machineIcon.alt = machine.displayName;
        machineIcon.loading = "lazy";
        let recipesTab = this.createHtmlElement("div", "recipes-tab");
        ...
        tabSelector.addEventListener("click", () => {
          this.dispatchEvent(new Event(_RecipeSelectionModal.recipesTabSwitchedEvent));
          recipesTab.classList.add("active");
          tabSelector.classList.add("active");
        });
        this.addEventListener(_RecipeSelectionModal.recipesTabSwitchedEvent, () => {
    ...
    createRecipesGroup(name) {
      let groupTitle = this.createHtmlElement("h3", "group-title");
      groupTitle.innerText = name;
      let groupDiv = this.createHtmlElement("div", "group");
      return { div: groupDiv, title: groupTitle };
    }
    appendRecipes(recipesGroup, eventsRecipesGroup, recipe, machine) {
      ...
      for (const product of recipe.products) {
        ...
        let resource = Satisfactory_default.resources.find((resource2) => {
    ...
    createResourceDisplay(parentDiv, craftingTime, recipeResource) {
      let descriptor = Satisfactory_default.resources.find(
        (resourceData) => {
    ...
    confirmRecipe() {
      if (this._selectedRecipe != void 0) {
        this.closeModal();
        this.dispatchEvent(new Event(_RecipeSelectionModal.recipeConfirmedEvent));
      }
    }
    closeModal() {
    ...
    createHtmlElement(tag, ...classList) {
      let element = document.createElement(tag);
    ...
    _isOpened = false;
    _selectedRecipe;
    _modalContainer = document.querySelector("#node-creation-container");
    _closeButton = this._modalContainer.querySelector(".title-row .close");
    _gameVersionText = this._modalContainer.querySelector("h2.title span.game-version");
    _tabSelectors = this._modalContainer.querySelector("#tab-selectors");
    _recipeTabs = this._modalContainer.querySelector("#recipe-tabs");
    _selectedRecipeDisplay = this._modalContainer.querySelector("#selected-recipe");
    _selectedRecipeName = this._selectedRecipeDisplay.querySelector("#selected-recipe-name>.text");
    _selectedRecipeMachine = this._selectedRecipeDisplay.querySelector("#selected-recipe-machine>.machine>img.icon");
    _selectedRecipeInput = this._selectedRecipeDisplay.querySelector("#selected-recipe-input");
    _selectedRecipeOutput = this._selectedRecipeDisplay.querySelector("#selected-recipe-output");
